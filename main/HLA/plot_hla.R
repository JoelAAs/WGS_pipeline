...
selected_subset = "Cns.Toxicity~rest"

hla_assoc_df <- read.csv("results/HLA_assoc.csv", sep="\t", as.is = T, check.names = F)
...
hla_assoc_df %>%  filter(TEST == "Cns.Toxicity~rest" & GENETIC_MODEL=="dominant") -> test
...
size=178/2
ggsave("plots/CNS_hla.png", g, width = size*((1+sqrt(5))/2), height = size, units="mm", dpi = 400)
