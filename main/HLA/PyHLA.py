...
def hlaFreq(caseAlleles, ctrlAlleles, np, nc, nn):
 ...
 ##       print caseAlleles
  #      print np, nc
        ...
        caseAll = caseAlleles[allele] if allele in caseAlleles else 0
        caseTot = np[allele.split('*')[0]] if allele.split('*')[0] in np else 0
        conAll  = ctrlAlleles[allele] if allele in ctrlAlleles else 0
        conTot  = nc[allele.split('*')[0]] if allele.split('*')[0] in nc else 0
        Tot     = 0.1 if caseTot + conTot == 0 else caseTot + conTot
		
        ff.append(1.0 * (conAll + caseAll) / Tot)
    ...
    for allele in caseAlleles.keys():
        if allele in ctrlAlleles:
            ...
            #        print n1, n2, n3, n4
                    ...
                    nn1 = 0.5 if n1 == 0 else n1
                    nn2 = 0.5 if n2 == 0 else n2
                    nn3 = 0.5 if n3 == 0 else n3
                    nn4 = 0.5 if n4 == 0 else n4
                    se = math.sqrt(1.0/nn1  + 1.0/nn2 +  1.0/nn3 + 1.0/nn4)
           #         print se, pvalue, OR
                    OR = 0.001 if OR == 0 else OR
...
def regressionLogistic(infile, digits, freq, model = 'additive', adjust = 'FDR', exclude=None, covfile=None, covname=None, perm=None, seed=None, test='logistic'):
    ...
    for allele in alleles:
        if allele not in excludeAlleles:
            ...
            f1 = 1.0 * n1 / n2 if n2 != 0 else 'NA'
            f2 = 1.0 * n3 / n4 if n4 != 0 else 'NA'
            f12 = 1.0 * (n1 + n3) / (n2 + n4)
            if f12 > freq:
                usedAllele.append(allele)
                n2 -= n1
                ...
                try:
                    myformula = myformula.replace('ö', 'o')
                    mydata.columns = mydata.columns.str.replace("ö", "o")
...
if SUMMARY:
    ...
    #    print myaln.keys()
     #   empty = [a for a in myaln if myaln[a] == []]
     #   print empty
        if PRINT:
            printAA(myaln)
